  ...
  export(selectedtable, options) {
    ...
    if (defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt') {
      ...
      var CollectCsvData = function($rows, rowselector, length) {
        $rows.each(function() {
          ...
          trData = jQuery.trim(trData).substring(0, trData.length - 1);
          if (trData.length > 0) {
      ...
      findTableElements(jQuery(el), 'tbody').each(function() {
        rowlength += CollectCsvData(findTableElements(thatTable, defaults.tbodySelector), 'td,th', rowlength);
      });
      ...
      $hrows = collectHeadRows(jQuery(el));
      jQuery($hrows).each(function() {
        ...
        rowIndex++;
        tdData = jQuery.trim(tdData).substring(0, tdData.length - 1);
      });
      tdData += ') VALUES ';

      // Data
      $rows = collectRows(jQuery(el));
      ...
      jQuery($rows).each(function() {
        ...
        if (jQuery.isEmptyObject(jsonObjectTd) === false) jsonArray.push(jsonObjectTd);
      ...
      jQuery(el)
        ...
        .each(function() {
          ...
          ssName = jQuery.trim(ssName.replace(/[\\\/[\]*:?'"]/g, '').substring(0, 31));
      ...
      jQuery(el)
        ...
        .each(function() {
          ...
          sheetName = jQuery.trim(sheetName.replace(/[\\\/[\]*:?'"]/g, '').substring(0, 31));
      ...
      jQuery(el)
        ...
        .each(function() {
          ...
          if (docName === '') {
            ...
            docName = jQuery.trim(docName.replace(/[\\\/[\]*:?'"]/g, '').substring(0, 31));
          }
          ...
          jQuery($rows).each(function() {
            ...
            ForEachVisibleCell(this, 'td,th', rowIndex, $hrows.length + $rows.length, function(cell, row, col) {
              if (cell !== null) {
                ...
                var tdstyle = '';
                var tdcss = jQuery(cell).attr('data-tableexport-msonumberformat');
          ...
          if (defaults.displayTableName)
            docData += '<tr><td></td></tr><tr><td></td></tr><tr><td>' + parseString(jQuery('<p>' + defaults.tableName + '</p>')) + '</td></tr>';
      ...
      html2canvas(jQuery(el)[0]).then(function(canvas) {
      ...
      if (defaults.pdfmake.enabled === true) {
        ...
        jQuery.extend(true, docDefinition, defaults.pdfmake.docDefinition);
        ...
        if (typeof pdfMake !== 'undefined') {
          ...
          if (pdfMake.vfs.hasOwnProperty('Mirza-Regular.ttf')) {
            ...
            jQuery.extend(true, pdfMake.fonts, {
              Mirza: { normal: 'Mirza-Regular.ttf', bold: 'Mirza-Bold.ttf', italics: 'Mirza-Medium.ttf', bolditalics: 'Mirza-SemiBold.ttf' },
            });
          } else if (pdfMake.vfs.hasOwnProperty('gbsn00lp.ttf')) {
            defaults.pdfmake.docDefinition.defaultStyle.font = 'gbsn00lp';
            jQuery.extend(true, pdfMake.fonts, {
              gbsn00lp: { normal: 'gbsn00lp.ttf', bold: 'gbsn00lp.ttf', italics: 'gbsn00lp.ttf', bolditalics: 'gbsn00lp.ttf' },
            });
          ...
          jQuery.extend(true, pdfMake.fonts, defaults.pdfmake.fonts);
        ...
        doc.addHTML(jQuery(el).first(), defaults.jspdf.margins.left, defaults.jspdf.margins.top, addHtmlOptions, function() {
        ...
        if (typeof defaults.jspdf.format === 'string' && defaults.jspdf.format.toLowerCase() === 'bestfit') {
          ...
          jQuery(el).each(function() {
            if (isVisible(jQuery(this))) {
              var w = getPropertyUnitValue(jQuery(this).get(0), 'width', 'pt');
        ...
        if (typeof teOptions.images !== 'undefined') {
          jQuery(el)
            ...
            .each(function() {
              ...
              if (defaults.exportHiddenCells === false) {
                $hiddenTableElements = jQuery(this)
                  .find('tr, th, td')
                  .filter(':hidden');
                checkCellVisibilty = $hiddenTableElements.length > 0;
              }

              $hrows = collectHeadRows(jQuery(this));
              $rows = collectRows(jQuery(this));

              jQuery($rows).each(function() {
                ForEachVisibleCell(this, 'td,th', $hrows.length + rowCount, $hrows.length + $rows.length, function(cell) {
                  collectImages(cell, jQuery(cell).children(), teOptions);
                });
                rowCount++;
              });
            });
        ...
        loadImages(teOptions, function() {
          jQuery(el)
            ...
            .each(function() {
              ...
              if (typeof teOptions.onTable === 'function') if (teOptions.onTable(jQuery(this), defaults) === false) return true; // continue to next iteration step (table)

              // each table works with an own copy of AutoTable options
              defaults.jspdf.autotable.tableExport = null; // avoid deep recursion error
              var atOptions = jQuery.extend(true, {}, defaults.jspdf.autotable);
              defaults.jspdf.autotable.tableExport = teOptions;

              atOptions.margin = {};
              jQuery.extend(true, atOptions.margin, defaults.jspdf.margins);
              ...
              if (typeof atOptions.createdHeaderCell !== 'function') {
                ...
                atOptions.createdHeaderCell = function(cell, data) {
                  ...
                  cell.styles = jQuery.extend({}, data.row.styles);
              ...
              if (typeof atOptions.drawCell !== 'function') {
                atOptions.drawCell = function(cell, data) {
                  ...
                  if (draw2canvas !== true) {
                    ...
                    var imgId = jQuery(container).attr('data-tableexport-canvas');
              ...
              if (rowIndex > 0) {
                ...
                while (lastrow >= 0) {
                  jQuery.each(teOptions.headerrows[lastrow], function() {
                    var obj = this;
              ...
              jQuery($rows).each(function() {
                ...
                ForEachVisibleCell(this, 'td,th', rowIndex, $hrows.length + $rows.length, function(cell, row, col) {
                  ...
                  if (typeof cell !== 'undefined' && cell !== null) {
                    ...
                    obj.elements = obj.isCanvas ? jQuery(cell) : jQuery(cell).children();

                    if (typeof jQuery(cell).data('teUserDefText') !== 'undefined') obj.hasUserDefText = true;

                    teOptions.teCells[rowCount + ':' + colKey++] = obj;
                  } else {
                    obj = jQuery.extend(true, {}, teOptions.teCells[rowCount + ':' + (colKey - 1)]);
                    obj.colspan = -1;
                    teOptions.teCells[rowCount + ':' + colKey++] = obj;
                  }
                });
              ...
              if (typeof teOptions.onBeforeAutotable === 'function') teOptions.onBeforeAutotable(jQuery(this), teOptions.columns, teOptions.rows, atOptions);

              teOptions.doc.autoTable(teOptions.columns, teOptions.rows, atOptions);

              // onAfterAutotable: optional callback function after returning
              // from jsPDF AutoTable that can be used to modify the AutoTable options
              if (typeof teOptions.onAfterAutotable === 'function') teOptions.onAfterAutotable(jQuery(this), atOptions);
    ...
    function collectHeadRows($table) {
      ...
      findTableElements($table, 'thead').each(function() {
        result.push.apply(result, findTableElements(jQuery(this), defaults.theadSelector).toArray());
      });
      return result;
    }
    ...
    function collectRows($table) {
      ...
      findTableElements($table, 'tbody').each(function() {
        result.push.apply(result, findTableElements(jQuery(this), defaults.tbodySelector).toArray());
      });
      if (defaults.tfootSelector.length) {
        findTableElements($table, 'tfoot').each(function() {
          result.push.apply(result, findTableElements(jQuery(this), defaults.tfootSelector).toArray());
        });
      }
      return result;
    }
    ...
    function GetColumnNames(table) {
      ...
      jQuery(table)
        ...
        .each(function(index, el) {
          if (jQuery(el).attr('data-field') !== undefined) result[index] = jQuery(el).attr('data-field');
    ...
    function isVisible($element) {
      ...
      if (isCell && tableexportDisplay !== 'none' && tableexportDisplay !== 'always') {
        $element = jQuery($element[0].parentNode);
    ...
    function isColumnIgnored($cell, rowLength, colIndex) {
      ...
      if (isVisible($cell)) {
        if (defaults.ignoreColumn.length > 0) {
          if (
            jQuery.inArray(colIndex, defaults.ignoreColumn) !== -1 ||
            jQuery.inArray(colIndex - rowLength, defaults.ignoreColumn) !== -1 ||
            (colNames.length > colIndex && typeof colNames[colIndex] !== 'undefined' && jQuery.inArray(colNames[colIndex], defaults.ignoreColumn) !== -1)
          )
            result = true;
        }
      } else result = true;
    ...
    function ForEachVisibleCell(tableRow, selector, rowIndex, rowCount, cellcallback) {
      if (typeof cellcallback === 'function') {
        ...
        if (typeof defaults.onIgnoreRow === 'function') ignoreRow = defaults.onIgnoreRow(jQuery(tableRow), rowIndex);

        if (
          ignoreRow === false &&
          (defaults.ignoreRow.length === 0 ||
            (jQuery.inArray(rowIndex, defaults.ignoreRow) === -1 && jQuery.inArray(rowIndex - rowCount, defaults.ignoreRow) === -1)) &&
          isVisible(jQuery(tableRow))
        ) {
          var $cells = findTableElements(jQuery(tableRow), selector);
          var cellCount = 0;
          ...
          $cells.each(function(colIndex) {
            ...
            jQuery.each(ranges, function() {
              ...
              if (rowIndex >= range.s.r && rowIndex <= range.e.r && cellCount >= range.s.c && cellCount <= range.e.c) {
                for (c = 0; c <= range.e.c - range.s.c; ++c) cellcallback(null, rowIndex, cellCount++);
              }
            });
            ...
            // Handle Colspan
            if (colspan) for (c = 0; c < colspan - 1; ++c) cellcallback(null, rowIndex, cellCount++);
          });
    ...
    function collectImages(cell, elements, teOptions) {
      if (typeof cell !== 'undefined' && cell !== null) {
        if (cell.hasAttribute('data-tableexport-canvas')) {
          ...
          jQuery(cell).attr('data-tableexport-canvas', imgId);
          ...
          elements.each(function() {
            ...
            collectImages(cell, jQuery(this).children(), teOptions);
          });
        }
      }
    }
    ...
    function loadImages(teOptions, callback) {
      ...
      function loadImage(image) {
        if (image.url) {
          if (!image.src) {
            var $imgContainer = jQuery(image.url);
    ...
    function drawAutotableElements(cell, elements, teOptions) {
      elements.each(function() {
        ...
        drawAutotableElements(cell, jQuery(this).children(), teOptions);
      });
    }
      ...
      if (typeof teOptions.onAutotableText === 'function') {
        ...
        if (texttags.length) {
          ...
          while (tag) {
            ...
            if (jQuery(tag).is('b')) b = true;
            else if (jQuery(tag).is('i')) i = true;
            ...
            if (b || i) {
              if (jQuery(tag).is('b')) b = false;
              else if (jQuery(tag).is('i')) i = false;
      ...
      if (cell !== null) {
        ...
        if ($cell[0].hasAttribute('data-tableexport-canvas')) {
          ...
          if (typeof defaults.onCellHtmlData === 'function') {
            ...
            var html = jQuery.parseHTML(htmlData);
            var inputidx = 0;
            var selectidx = 0;
            ...
            jQuery.each(html, function() {
              if (jQuery(this).is('input')) {
                ...
                htmlData += $cell
                  ...
                  .eq(selectidx++)
                  .text();
              } else if (jQuery(this).is('br')) {
                htmlData += '<br>';
              } else {
                if (typeof jQuery(this).html() === 'undefined') htmlData += jQuery(this).text();
                else if (
                  jQuery().bootstrapTable === undefined ||
                  (jQuery(this).hasClass('fht-cell') === false && // BT 4
                    jQuery(this).hasClass('filterControl') === false &&
                    $cell.parents('.detail-view').length === 0)
                )
                  htmlData += jQuery(this).html();

                if (jQuery(this).is('a')) {
                  var href = $cell.find('a').attr('href') || '';
                  if (typeof defaults.onCellHtmlHyperlink === 'function') result += defaults.onCellHtmlHyperlink($cell, rowIndex, colIndex, href, htmlData);
        ...
        if (htmlData && htmlData !== '' && defaults.htmlContent === true) {
          ...
          if (cellFormat !== '') {
            ...
            jQuery.each(obj.text().split('\u2028'), function(i, v) {
              if (i > 0) text += ' ';
    ...
    function getColspan(cell) {
      var result = jQuery(cell).attr('data-tableexport-colspan');
      if (typeof result === 'undefined' && jQuery(cell).is('[colspan]')) result = jQuery(cell).attr('colspan');

      return parseInt(result) || 0;
    }

    function getRowspan(cell) {
      var result = jQuery(cell).attr('data-tableexport-rowspan');
      if (typeof result === 'undefined' && jQuery(cell).is('[rowspan]')) result = jQuery(cell).attr('rowspan');
    ...
    function xlsxTableToSheet(table) {
      ...
      for (; _R < rows.length && R < sheetRows; ++_R) {
        ...
        var ignoreRow = false;
        if (typeof defaults.onIgnoreRow === 'function') ignoreRow = defaults.onIgnoreRow(jQuery(row), _R);

        if (
          ignoreRow === true ||
          (defaults.ignoreRow.length !== 0 && (jQuery.inArray(_R, defaults.ignoreRow) !== -1 || jQuery.inArray(_R - rows.length, defaults.ignoreRow) !== -1)) ||
          isVisible(jQuery(row)) === false
        ) {
          continue;
        }
        ...
        for (_C = C = 0; _C < elts.length; ++_C) {
          ...
          var col = _C + CSOffset;
          if (isColumnIgnored(jQuery(elt), _CLength, col + (col < C ? C - col : 0))) continue;
          CSOffset += CS - 1;
          ...
          var o = { t: 's', v: v };
          var _t = '';
          var cellFormat = jQuery(elt).attr('data-tableexport-cellformat');

          if (cellFormat !== '') {
            var ssfId = parseInt(jQuery(elt).attr('data-tableexport-xlsxformatid') || 0);

            if (ssfId === 0 && typeof defaults.mso.xslx.formatId.numbers === 'function')
              ssfId = defaults.mso.xslx.formatId.numbers(jQuery(elt), _R, _C + CSOffset);

            if (ssfId === 0 && typeof defaults.mso.xslx.formatId.date === 'function') ssfId = defaults.mso.xslx.formatId.date(jQuery(elt), _R, _C + CSOffset);
          ...
          if (v != null) {
            ...
            if (v.length === 0) o.t = 'z';
            ...
            else if (_t === '' && jQuery(elt).find('a').length) {
              ...
              if (ssfId === 0 && typeof defaults.mso.xslx.formatId.numbers !== 'function') ssfId = defaults.mso.xslx.formatId.numbers;
    ...
    function saveToFile(data, fileName, type, charset, encoding, bom) {
      ...
      if (saveIt) {
        try {
          ...
          downloadFile(
            fileName,
            'data:' + type + (charset.length ? ';charset=' + charset : '') + (encoding.length ? ';' + encoding : '') + ',',
            bom ? '\ufeff' + data : data
          );
        }
      }
